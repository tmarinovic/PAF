...
class EMField:
    ...
    def init(self,m,q,r,v,E,B,dt):
        ...
        self.E = E
        self.B = B
        self.a = self.akceleracija(self.v)
        self.dt = dt
        self.t = 0
    ...
    def __runge_kutta(self):
        k1v = self.akceleracija(self.v)*self.dt 
        k1r = self.v*self.dt
        k2v = self.akceleracija(np.add(self.v,k1v/2))*self.dt
        k2r = (np.add(self.v,k1v/2))*self.dt
        k3v = self.akceleracija(np.add(self.v,k2v/2))*self.dt
        k3r = (np.add(self.v,k2v/2))*self.dt
        k4v = self.akceleracija(np.add(self.v,k3v))*self.dt
